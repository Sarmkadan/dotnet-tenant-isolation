...
namespace TenantIsolation.Models
{
    ...
    public static class TenantConnectionStringExtensions
    {
        ...
        public static void Validate(this TenantConnectionString connectionString)
        {
            ...
            if (string.IsNullOrEmpty(connectionString.ServerHost))
                throw new ArgumentException("ServerHost must not be null or empty.", nameof(connectionString.ServerHost));

            if (connectionString.ServerPort is null or <= 0 or > 65535)
                throw new ArgumentException("ServerPort must be between 1 and 65535.", nameof(connectionString.ServerPort));

            if (string.IsNullOrEmpty(connectionString.DatabaseType))
                throw new ArgumentException("DatabaseType must not be null or empty.", nameof(connectionString.DatabaseType));

            if (connectionString.DatabaseType is not ("SqlServer" or "PostgreSQL" or "MySQL"))
                throw new ArgumentException("DatabaseType must be a supported type: SqlServer, PostgreSQL, MySQL.", nameof(connectionString.DatabaseType));

            if (!string.IsNullOrEmpty(connectionString.SchemaName) && connectionString.SchemaName.Length > 128)
                throw new ArgumentException("SchemaName must not exceed 128 characters.", nameof(connectionString.SchemaName));
        }
